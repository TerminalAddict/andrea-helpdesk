...
CREATE TABLE IF NOT EXISTS agents (
    ...
    notification_preferences_json TEXT NULL COMMENT 'Per-agent notification type preferences as JSON',
    chat_handle         VARCHAR(80) NULL COMMENT 'Stable lowercase @mention handle for internal chat',
    ...
    UNIQUE KEY uq_agents_chat_handle (chat_handle),
...
CREATE TABLE IF NOT EXISTS tickets (
    ...
    priority            ENUM('low','normal','high','urgent','overdue') NOT NULL DEFAULT 'normal',
    due_at              DATETIME     NULL DEFAULT NULL COMMENT 'Due date/time (NULL = no due date)',
    due_end             DATETIME     NULL DEFAULT NULL COMMENT 'End of multi-day or time-range due date',
    ...
    email_delivery_failed_at DATETIME NULL COMMENT 'Most recent outbound delivery failure detected from an inbound bounce',
    email_delivery_failed_recipient VARCHAR(255) NULL COMMENT 'Recipient address from the most recent outbound delivery failure',
    email_delivery_failed_summary VARCHAR(255) NULL COMMENT 'Short summary of the most recent outbound delivery failure',
    first_response_at   DATETIME NULL COMMENT 'Time of first agent reply (SLA)',
    last_attention_at   DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Most recent customer or agent activity used for SLA escalation',
    sla_high_notified_at DATETIME NULL COMMENT 'Last time the high-priority SLA reminder was sent for the current inactivity cycle',
    sla_overdue_notified_at DATETIME NULL COMMENT 'Last time the overdue SLA reminder was sent for the current inactivity cycle',
    ...
    INDEX idx_tickets_attention (last_attention_at),
